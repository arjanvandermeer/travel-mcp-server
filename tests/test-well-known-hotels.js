...
/**
 * Test well-known hotels in Bangkok to verify Google Places enrichment
 */
...
async function test() {
  console.log('=== Testing Well-Known Hotels in Bangkok ===\n');

  const db = new TravelDatabase();

  try {
    console.log('1. Searching for hotels with "Marriott" in Bangkok...');
    ...
    if (result.pois && result.pois.length > 0) {
      ...
      console.log('3. Waiting 15 seconds for background enrichment...');
      ...
      enrichmentCheck.rows.forEach((row, i) => {
        ...
        if (row.google_rating) {
          ...
          console.log(`      Address: ${row.google_formatted_address}`);
        }
      });
