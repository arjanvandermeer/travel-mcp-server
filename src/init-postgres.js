...
const CONNECTION_STRING = 'postgresql://traveluser:travelpass@localhost:5432/travel';
  ...
  try {
    ...
    const functionsResult = await client.query(`
      ...
      WHERE routine_schema = 'public'
        ...
        AND routine_name LIKE 'find_hotels%'
    ...
    const stats = await Promise.all([
      ...
      client.query('SELECT COUNT(*) FROM hotels'),
      client.query('SELECT COUNT(*) FROM regions'),
    ]);

    console.log(`  - Countries: ${stats[0].rows[0].count}`);
    console.log(`  - Cities: ${stats[1].rows[0].count}`);
    console.log(`  - Hotels: ${stats[2].rows[0].count}`);
    console.log(`  - Regions: ${stats[3].rows[0].count}`);

    console.log('\n✅ Database initialization complete!');
    console.log('\nNext steps:');
    console.log('  1. Run: node src/migrate-geonames.js (migrate GeoNames data from SQLite)');
    console.log('  2. Run: node src/import-osm-pbf.js (import OSM hotel data from PBF files)');
