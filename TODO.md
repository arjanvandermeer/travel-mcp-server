...
- [ ] **Medium** `src/database.js`: File is about 2,350 lines after extracting response helpers to `src/response-utils.js` and user/auth/favorite methods to `src/database-user-methods.js`. Stale pending enrichment restarts now have a cooldown with regression coverage. Recommended next action: split by domain when touching related code, starting with POI search/enrichment and import/logging helpers.
- [ ] **Medium** `src/tools-config.js`: File is about 900 lines after extracting POI view/nearby helpers to `src/poi-view-utils.js`, MCP prompt definitions to `src/prompts-config.js`, and MCP resource handling to `src/resources-config.js`. Recommended next action: split large tool schema groups and handler wiring into focused modules while preserving the exported contract.
